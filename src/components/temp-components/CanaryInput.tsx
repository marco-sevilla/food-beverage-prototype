...
const CanaryInput = forwardRef<HTMLInputElement, CanaryInputProps>(
  (
    ...
    const inputClasses = clsx(
      ...
      // Left addon padding
      leftAddon && "pl-10",
      // Right side padding based on what's shown
      rightAddon && !isAIFilled && "pr-10",
      isAIFilled && "pr-10",
      rightAddon && isAIFilled && "pr-14",
      className
    );

    const inputStyles = {
      borderColor: error ? colors.red1 : (isDisabled ? colors.black1 : colors.black3),
    ...
    return (
      <div className="w-full">
        {label && (
          ...
          >
            {label}
            {isRequired && <span className="ml-1" style={{ color: colors.red1 }}>*</span>}
          </label>
        )}
        ...
        {error && (
          <div className="mt-1 inline-block px-2 py-[2px] rounded" style={{ backgroundColor: colors.black6 }}>
            <p className="text-[12px] leading-[1.5]" style={{ color: colors.red1 }}>
              {error}
            </p>
          </div>
        )}
