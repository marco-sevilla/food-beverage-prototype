...
export default function CanaryTag({
  ...
  return (
    <div
      className={`inline-flex items-center justify-center gap-1 rounded-[2px] font-medium w-max ${
        uppercase ? 'uppercase' : ''
      } ${variant === TagVariant.OUTLINE ? 'border border-solid' : ''} ${sizeClasses} ${className}`}
      style={{
        backgroundColor,
